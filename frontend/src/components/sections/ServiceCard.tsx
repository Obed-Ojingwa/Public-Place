...
// Path: src/components/sections/ServiceCard.tsx
...
interface ServiceCardProps {
  ...
  href: string;
  benefits?: string[];
  ctaText?: string; // Optional override — defaults to "Explore {title}"
}
...
export default function ServiceCard({
  ...
  const linkLabel = ctaText ?? `Explore ${title}`;
  ...
  return (
    <div className="group relative bg-white rounded-xl border border-slate-200 p-8 shadow-sm hover:shadow-lg transition-all duration-300">
      ...
      {/* CTA — descriptive text for SEO + accessibility */}
      <Link
        href={href}
